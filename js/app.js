    ...
    (() => {
        ...
        function onEntry(entry) {
            entry.forEach((change => {
                if (change.isIntersecting) change.target.classList.add("showed");
            }));
        }
        let options = {
            threshold: [ .5 ]
        ...
        let script_elements = document.querySelectorAll("section");
        for (let elm of script_elements) observer.observe(elm);
        function scrollAnimation(block) {
            window.addEventListener("scroll", (() => {
                if (document.querySelector(`.${block}`).classList.contains("showed")) {
                    let parrenrBlock = document.querySelector(`.${block}`);
                    let showedItem = parrenrBlock.querySelectorAll(".showed-item");
                    let showedDaley = 200;
                    showedItem.forEach((item => {
                        showedDaley += 500;
                        setTimeout((() => item.classList.add("_active")), showedDaley);
                    }));
                }
            }));
        }
        scrollAnimation("resons");
        scrollAnimation("services");
        let headerNav = document.querySelector(".header_nav"), headerLang = document.querySelector(".header_lang"), mobileMenu = document.querySelector(".mobile__menu");
        if (window.innerWidth < 1050) {
            headerNav.remove();
            headerLang.remove();
            mobileMenu.appendChild(headerNav);
            mobileMenu.appendChild(headerLang);
            new Swiper(".services-container", {
                spaceBetween: 30,
                pagination: {
                    el: ".swiper-pagination.sevices",
                    clickable: true
                },
                breakpoints: {
                    320: {
